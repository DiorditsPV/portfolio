...
## General — библиотека разборов

`presentations/` — html-страницы, которые делает скилл `present-html`, и страница-библиотека
`presentations/index.html` поверх них. Адрес — `paveldiordits.site/presentations/`, пункт **General**
в шапке сайта.
...
- `--slug` — имя в URL. Обязателен: имена файлов в `~/dev/docs` не годятся (там лежат `review.html`
  и `fix_report.html`).
- `--topic` — направление; красит маркер в карточке и обложку. Список — в `TOPICS` внутри скрипта.
...
- `--pick` — метка «Лучшее» на карточке. `--feature` её включает автоматически.
- `--unlisted` — «по ссылке»: файл уезжает на сервер и открывается по прямому адресу, но в библиотеке
  не показывается. «Черновик» и «приватная» — это просто неопубликованный файл, он остаётся в `~/dev/docs`.
- Заголовок берётся из `<title>`, время просмотра считается по числу слов, а заголовки разделов уходят
  в поисковый индекс — поэтому команда работает и со старыми файлами, ничего в них не правя.
...
- **Дописки в копию.** В скопированный html добавляются og-теги (иначе ссылка в телеграме выглядит
  голой) и подвал с возвратом в библиотеку и блоком «ещё по теме». Всё между маркерами
  `<!--general:head-->` и `<!--general:foot-->`; перед каждой сборкой прошлая вставка вырезается,
  поэтому пересборка не наслаивает блоки. Исходник в `~/dev/docs` не трогается.
...
живут в одном месте, и General не может разъехаться с главной. Правишь `:root` в `index.html` —
...
Саму страницу руками не правь, она генерируется: шаблон — `tools/gallery-page.mjs`, обложки —
`tools/cover.mjs`.
...
требовало правки выкладки: когда vhost появится, меняются только `BASE` в скрипте и `href` пункта
`General` в `T.nav`, дальше `--rebuild`.
